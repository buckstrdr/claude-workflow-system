...
**MANDATORY**: Always run Playwright UI verification when frontend changes

```bash
#!/usr/bin/env bash
# E2E UI Verification with Playwright

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  E2E Verification: UI (Playwright)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""
...
echo ""
echo "This will:"
echo "  1. Navigate to UI with Playwright"
echo "  2. Capture console logs"
echo "  3. Take full-page screenshot (1920x1080)"
echo "  4. Verify key UI elements"
echo "  5. Save report to .ian/"
echo ""

# Call /ui-verify skill
# (In actual usage, you would run: /ui-verify)
# For automated e2e, invoke the skill steps here

# Navigate to UI
echo "  → Navigating to http://localhost:5173..."
...
echo "  → Analyzing console logs..."
# Console messages are automatically captured in Playwright response
# Look for [ERROR] messages that aren't expected (ERR_CONNECTION_REFUSED during startup is OK)

CRITICAL_ERRORS=$(echo "$PLAYWRIGHT_CONSOLE" | grep -E "\[ERROR\].*TypeError|\[ERROR\].*ReferenceError|\[ERROR\].*Uncaught" | grep -v "ERR_CONNECTION_REFUSED" || echo "")

if [ -n "$CRITICAL_ERRORS" ]; then
  echo "❌ UI: Critical console errors found"
  echo "$CRITICAL_ERRORS"
  EXIT_CODE=1
else
  echo "✓ UI: No critical console errors"
fi
...
mcp__playwright__browser_take_screenshot({
  filename: ".ian/e2e_ui_verify_${TIMESTAMP}.png",
  type: "png",
  fullPage: true
})

echo "✓ UI: Screenshot saved to .playwright-mcp/.ian/e2e_ui_verify_${TIMESTAMP}.png"

# Verify key elements present
echo "  → Verifying key UI elements..."
...
for element in "${KEY_ELEMENTS[@]}"; do
  if ! echo "$PLAYWRIGHT_SNAPSHOT" | grep -q "$element"; then
    MISSING_ELEMENTS+=("$element")
  fi
done

if [ ${#MISSING_ELEMENTS[@]} -gt 0 ]; then
  echo "❌ UI: Missing elements:"
  printf '  - %s\n' "${MISSING_ELEMENTS[@]}"
  EXIT_CODE=1
else
  echo "✓ UI: All key elements present"
fi
...
echo ""
echo "Results:"
echo "  - Console logs: Checked"
echo "  - Screenshot: .playwright-mcp/.ian/e2e_ui_verify_${TIMESTAMP}.png"
echo "  - UI elements: Verified"
echo ""
```

**IMPORTANT**: This section uses Playwright MCP tools:
- `mcp__playwright__browser_navigate` - Navigate to UI
- `mcp__playwright__browser_take_screenshot` - Capture screenshot
- Console logs are automatically captured
- Page snapshot (YAML) shows all UI elements

**When to skip**: If changes are backend-only, UI verification is optional. Otherwise, MANDATORY.
